...
class Subscriptionreturn extends \Magento\Framework\App\Action\Action {
    ...
    public function execute() {
        ...
        $currentUser = $objectManager->get ( 'Magento\Customer\Model\Session' );
         ...
         */
        $subscriptionProfilesModel = $objectManager->get ( 'Apptha\Marketplace\Model\Subscriptionprofiles' )->load ( $invoiceId, 'invoice' );
        ...
        if (count ( $subscriptionProfilesModel )) {
            ...
            if ($emailNotificationEnabled == 1) {
                ...
                $currency = $objectManager->get ( 'Magento\Store\Model\StoreManagerInterface' )->getStore ()->getBaseCurrencyCode ();
                 ...
                 */
                $objectManager->get ( 'Apptha\Marketplace\Helper\Email' )->yourCustomMailSendMethod ( $emailTempVariables, $receiverInfo, $senderInfo, $templateId );
            }
