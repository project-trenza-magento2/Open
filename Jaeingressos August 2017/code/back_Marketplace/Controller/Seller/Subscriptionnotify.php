...
class Subscriptionnotify extends \Magento\Framework\App\Action\Action {
    ...
    public function execute() {
        ...
        $actionUrl = $objectManager->get ( 'Apptha\Marketplace\Block\Seller\Subscriptionplans' )->getActionUrl ();
        ...
        if (strcmp ( $result, "VERIFIED" ) == 0) {
             ...
             */
            $subscriptionProfilesModel = $objectManager->get ( 'Apptha\Marketplace\Model\Subscriptionprofiles' )->load ( $invoice, 'invoice' );
    ...
    public function updateProductStatus($sellerId, $action, $maxProductCount) {
         ...
         */
        $objectManager = \Magento\Framework\App\ObjectManager::getInstance ();
        $product = $objectManager->get ( 'Magento\Catalog\Model\Product' )->getCollection ()->addFieldToFilter ( 'seller_id', $sellerId );
        ...
        foreach ( $sellerProductIds as $sellerProductId ) {
            ...
            if ($maxProductCount >= $productCount && $action == 'enable' || $maxProductCount == 'unlimited' && $action == 'enable') {
                $objectManager->get ( 'Magento\Catalog\Model\Product' )->load ( $sellerProductId )->setProductApproval ( 1 )->save ();
            } else {
                $objectManager->get ( 'Magento\Catalog\Model\Product' )->load ( $sellerProductId )->setProductApproval ( 0 )->setStatus ( 0 )->save ();
            }
            /**
