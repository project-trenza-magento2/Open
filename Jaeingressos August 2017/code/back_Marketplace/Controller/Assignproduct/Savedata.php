...
class Savedata extends \Magento\Framework\App\Action\Action {
    ...
    public function execute() {
        ...
        if ($baseTypeId == "configurable") {
            ...
            foreach ( $variants as $variant ) {
                ...
                if (! in_array ( $variant, $childrenProductIds )) {
                    $newProducts [] = $variant;
                    $productData = $this->_objectManager->create ( 'Magento\Catalog\Model\Product' )->load ( $variant );
                ...
                $configurableProduct = $this->_objectManager->create ( 'Magento\Catalog\Model\Product' )->load ( $parentId );
    ...
    public function saveBaseProductInfo ( $parentId, $assignProd,$nationalShippingAmount,$internationalShippingAmount) {
        $baseProduct = $this->_objectManager->create ( 'Magento\Catalog\Model\Product' )->load ( $parentId );
