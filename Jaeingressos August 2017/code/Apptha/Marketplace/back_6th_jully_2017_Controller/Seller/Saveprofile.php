...
class Saveprofile extends \Magento\Framework\App\Action\Action {
    ...
    public function execute() {
        ...
        if ($customerSession->isLoggedIn ()) {
            ...
            //echo $sellerDetails->getEmail();
            
            
            $objectManager = \Magento\Framework\App\ObjectManager::getInstance(); 
            $customer_obj = $objectManager->get('Magento\Customer\Model\ResourceModel\CustomerRepository');
             $currPass = $this->getRequest()->getPost('current_password');
             $newPass = $this->getRequest()->getPost('password');
             $confPass = $this->getRequest()->getPost('password_confirmation');
            //$customer = $customer_obj->getById($customerId);
            //$customer_obj->save($customer, $this->encryptor->getHash($newPass, true));
...
$customerInterface = $objectManager->get('\Magento\Customer\Api\AccountManagementInterface'); 
...
if ($customer->getId() && false) {
    ...
    /*
    echo $hash= $customer->getPasswordHash();echo '<br>';
    $hashPassword = explode(':', $hash);
    echo $firstPart = $hashPassword[0];echo '<br>';
    echo $salt = $hashPassword[1]; 
    $pass = 'Open12345';
    ...
    //$magento_hash = $objectManager->get(/app/code/core/Mage/Core/Model/Encryption);
    //exit;
    //$customer = $customerInterface->authenticate($email, $currPass);
    //exit;
    $save_customer = $customerInterface->changePassword($email, $currPass, $newPass);
    /*
    try {
        $password_change = 0;
        $save_customer = $customerInterface->changePassword($email, $currPass, $newPass);
        ...
        $password_change = 0;
        $this->messageManager->addSuccess ( __ ( 'Password Error' ) ); 
    } */
   
   // set store as 
   /*
   $customer->setPassword('#Open123#');
    try {
		...
		echo 'wrong';
        $this->messageManager->addError ( __ ( 'Error saving Please try again later' ) );
    }*/
}
            $sellerDetails->save ();
    ...
    /**
    protected function changeCustomerPassword($email,$currPass,$newPass)
    {
        ...
        if ($this->getRequest()->getParam('change_password')) {
            ...
            try {
            $isPasswordChanged = $customerInterface->changePassword($email, $currPass, $newPass,$confPass);
            }catch (Exception $ex) {
            echo 'wrong';
            }
        }
