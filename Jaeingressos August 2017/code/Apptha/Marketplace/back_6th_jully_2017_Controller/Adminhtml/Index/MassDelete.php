...
class MassDelete extends \Magento\Customer\Controller\Adminhtml\Index\MassDelete {
    ...
    protected function massAction(AbstractCollection $collection) {
        ...
        $customerModel = $objectManager->get ( 'Apptha\Marketplace\Model\Seller' );
        ...
        foreach ( $collection->getAllIds () as $customerId ) {
            ...
            if (in_array ( $customerId, $customerIds )) {
                $sellerProductCollection = $objectManager->get ( 'Magento\Catalog\Model\Product' )->getCollection ()->addAttributeToFilter ( 'seller_id', $customerId );
                ...
                foreach ( $sellerProductDatas as $sellerProducts ) {
                    ...
                    $objectManager->create ( 'Magento\Catalog\Model\Product' )->load ( $productId )->setStatus ( 2 )->setProductApproval ( 0 )->save ();
                }
                $seller = $objectManager->get ( 'Apptha\Marketplace\Model\Seller' )->load ( $customerId, 'customer_id' );
                $seller->delete ();
            }
            $customersDeleted ++;
        }
