...
class Order extends \Magento\Framework\App\Action\Action {
    ...
    public function __construct(\Magento\Framework\App\Action\Context $context, \Magento\Framework\View\Result\PageFactory $resultPageFactory, \Magento\Framework\Message\ManagerInterface $messageManager) {
    ...
    public function execute() {
        ...
        $sellerOrderItems = $objectManager->get ( 'Apptha\Marketplace\Model\Orderitems' )->load ( $id, 'order_item_id' );
        ...
        if ($sellerOrderItems->getSellerId () == $sellerId) {
            ...
            if ($action == 'canceled' || $action == 'returned') {
                ...
                $orderItems = $objectManager->get ( 'Apptha\Marketplace\Model\Orderitems' )->getCollection ();
                ...
                if ($orderItemCount == $canceledOrderItemCount) {
                    $sellerOrder = $objectManager->get ( 'Apptha\Marketplace\Model\Order' )->getCollection ();
                    ...
                    $updateOrderStatus = $objectManager->get ( 'Apptha\Marketplace\Model\Order' )->load ( $sellerOrderId );
                    if ($action == 'canceled') {
                 ...
                 */
                $order = $objectManager->get ( 'Magento\Sales\Model\Order' )->load ( $sellerOrderItems->getOrderId () );
                
                $allItems = $order->getAllItems ();
                $allItemsCount = count ( $allItems );
                
                $allSellerOrderItem = $objectManager->get ( 'Apptha\Marketplace\Model\Orderitems' )->getCollection ();
    ...
    public function sendCancelOrRefundEmail($customerSession, $sellerOrderItems, $action) {
         ...
         * Get customer data
         */
        $buyerData = $objectManager->create ( 'Magento\Customer\Model\Customer' )->load ( $sellerOrderData->getCustomerId () );
