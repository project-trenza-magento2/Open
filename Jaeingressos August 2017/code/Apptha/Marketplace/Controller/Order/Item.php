...
class Item extends \Magento\Framework\App\Action\Action {
    ...
    public function execute() {
        ...
        if ($orderCustomerId == $customerId && $customerId != '') {
             ...
             */
            $sellerOrderItems = $objectManager->get ( 'Apptha\Marketplace\Model\Orderitems' )->getCollection ()->addFieldToFilter ( 'order_id', $orderId )->addFieldToFilter ( 'seller_id', $sellerId )->addFieldToFilter ( 'product_id', $productId )->getFirstItem ();
            
            /**
             * Change order item status
             */
            if (count ( $sellerOrderItems ) >= 1) {
                $orderItems = $objectManager->get ( 'Apptha\Marketplace\Model\Orderitems' )->load ( $sellerOrderItems->getId () );
             ...
             */
            $emailTemplateVariables ['requesturl'] = $objectManager->get ( 'Magento\Store\Model\StoreManagerInterface' )->getStore ( $storeId )->getBaseUrl () . 'marketplace/order/vieworder/id/' . $orderId;
            /**
