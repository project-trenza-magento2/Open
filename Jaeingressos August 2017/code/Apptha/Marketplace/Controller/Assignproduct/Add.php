...
class Add extends \Magento\Framework\App\Action\Action {
    ...
    public function checkSubscriptionPlan() {
        ...
        $loggedInUser = $obj->get ( 'Magento\Customer\Model\Session' );
        $sellerId = $loggedInUser->getId ();
        $checkSubscriptionPlans = $obj->get ( 'Apptha\Marketplace\Helper\Data' )->isSellerSubscriptionEnabled ();
        if ($checkSubscriptionPlans) {
            $currentDate = $obj->get ( 'Magento\Framework\Stdlib\DateTime\DateTime' )->gmtDate ();
            /**
             * To count subscription profiles
             */
            $subscribedData = $obj->get ( 'Apptha\Marketplace\Model\Subscriptionprofiles' )->getCollection ();
            ...
            if (count ( $subscribedData )) {
                ...
                $product = $obj->get ( 'Magento\Catalog\Model\Product' )->getCollection ()->addFieldToFilter ( 'seller_id', $sellerId );
