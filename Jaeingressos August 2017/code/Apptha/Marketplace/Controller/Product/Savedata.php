    ...
    public function execute() {
      ...
      if ($productTypeId == 'downloadable') {
        ...
        if (! empty ( $productId )) {
            ...
            $objectGroupManager = \Magento\Framework\App\ObjectManager::getInstance ();
            $product_name_url = str_replace(' ', '-', $productData ['name']);
            $productCollection = $objectGroupManager->create ( 'Magento\Catalog\Model\ResourceModel\Product\Collection' )->addAttributeToFilter ( 'url_key',$product_name_url);
            $productCollectionData = $productCollection->getData ();
            $urlKeyCount = count ( $productCollectionData );
            if ($urlKeyCount >= 1) {
                $product->setUrlKey ( $product_name_url . $sku_data );
            }
