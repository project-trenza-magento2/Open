...
class LoginPost extends \Magento\Customer\Controller\AbstractAccount {
    ...
    public function execute() {
        if ($this->session->isLoggedIn () || ! $this->formKeyValidator->validate ( $this->getRequest () )) {
            ...
            $resultRedirect->setPath ( 'marketplace/product/add' );
        ...
        $customer_group = $this->session->getCustomer()->getGroupId();
        if($customer_group == 1){
            return $this->resultRedirectFactory->create()->setPath('marketplace/general/changebuyer', ['_current' => true]);
        }elseif($customer_group == 4){
            return $this->resultRedirectFactory->create()->setPath('marketplace/product/add', ['_current' => true]);
        }else{
